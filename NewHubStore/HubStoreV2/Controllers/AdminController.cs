...
using CoreOne.HubStore.Dtos.UserDetail;
using CoreOne.HubStore.ViewModels;
using DevExtreme.AspNet.Data;
using HubStore.Models;
...
namespace HubStore.Controllers
{
    public class AdminController : Controller
    {
        ...
        public IActionResult UserProfile()
        {
            ...
            newUser.UserSerial = _identityContext.Users.OrderByDescending(o => o.UserSerial).Select(s => s.UserSerial).Max() + 1;
        ...
        public async Task<IActionResult> UserProfile(string id)
        {
            ...
            var user = new UserDto
            {
                ...
                UserSerial = entityUser.UserSerial
            };
        ...
        public async Task<IActionResult> UserProfile(UserDto user)
        {
            ...
            if (string.IsNullOrEmpty(user.Id))
            {
                ...
                user.UserSerial = _identityContext.Users.Any()
                    ? _identityContext.Users.Max(x => x.UserSerial) + 1
                    : 1;

                // إنشاء كيان المستخدم
                var hasher = new PasswordHasher<UserIdentity>();
                var userEntity = new UserIdentity
                {
                    Id = Guid.NewGuid().ToString(),
                    UserName = user.UserName,
